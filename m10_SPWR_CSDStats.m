...
for iGroup = [1:4 6] % For each experimental setup
    ...
    if iGroup == 1 || iGroup == 2 || iGroup == 3 || iGroup == 6
        ...
        a2nums.rPre = [a2nums.rPre rPre];
        a2nums.rRip = [a2nums.rRip rRip];
        a2nums.rPost = [a2nums.rPost rPost];
        
        labels2l = [labels2l; repmat({treat,state},length(lPre),1)];
        labels2r = [labels2r; repmat({treat,state},length(rPre),1)];
    end
...
[~,lPreT,lPreStats2] = anovan(a2nums.lPre,{labels2l(:,1),labels2l(:,2)},'model','interaction', 'Display','off');
[~,lRipT,lRipStats2] = anovan(a2nums.lRip,{labels2l(:,1),labels2l(:,2)},'model','interaction', 'Display','off');
[~,lPostT,lPostStats2] = anovan(a2nums.lPost,{labels2l(:,1),labels2l(:,2)},'model','interaction', 'Display','off');

[~,rPreT,rPreStats2] = anovan(a2nums.rPre,{labels2r(:,1),labels2r(:,2)},'model','interaction','Display','off');
[~,rRipT,rRipStats2] = anovan(a2nums.rRip,{labels2r(:,1),labels2r(:,2)},'model','interaction','Display','off');
[~,rPostT,rPostStats2] = anovan(a2nums.rPost,{labels2r(:,1),labels2r(:,2)},'model','interaction','Display','off');
